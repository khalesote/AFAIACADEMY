# Script para agregar la portada al ZIP optimizado
...
$rutaPortada = Join-Path $rutaBase "PORTADA_CodigoFuente_Optimizado.txt"

Write-Host "Agregando portada al ZIP optimizado..." -ForegroundColor Green
...
if (-not (Test-Path $rutaPortada)) {
    Write-Error "No se encuentra la portada: PORTADA_CodigoFuente_Optimizado.txt"
    exit 1
}
...
$tempDir = Join-Path $env:TEMP "zip_optimizado_temp_$(Get-Random)"
