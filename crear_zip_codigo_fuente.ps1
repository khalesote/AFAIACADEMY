# Script para crear ZIP del código fuente para registro de propiedad intelectual
# Excluye archivos compilados, dependencias y archivos temporales
...
Write-Host "Creando archivo ZIP del código fuente..." -ForegroundColor Green
...
$tempDir = Join-Path $env:TEMP "codigo_fuente_temp_$(Get-Random)"
...
try {
    ...
    Write-Host "Tamaño del archivo: $([math]::Round((Get-Item $rutaZip).Length / 1MB, 2)) MB" -ForegroundColor Cyan
